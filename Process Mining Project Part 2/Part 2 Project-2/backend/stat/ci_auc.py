...
updated on 9/1/2023: Daniel Quezada -> updated np.float to np.double
"""
...
def fastDeLong_weights(predictions_sorted_transposed, label_1_count, sample_weight):
    ...
    tx = np.empty([k, m], dtype=np.double)
    ty = np.empty([k, n], dtype=np.double)
    tz = np.empty([k, m + n], dtype=np.double)
    for r in range(k):
